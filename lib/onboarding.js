...
export async function transferAnonData(anonUserId, courseId = null) {
    try {
        const resolvedAnonUserId = anonUserId || readAnonUserId();
        if (!resolvedAnonUserId && !courseId) return null;

        const res = await authFetch(`${API_PREFIX}/transfer-data`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                ...(resolvedAnonUserId ? { anonUserId: resolvedAnonUserId } : {}),
                ...(courseId ? { courseId } : {}),
            }),
        });
        const body = await res.json().catch(() => ({}));
