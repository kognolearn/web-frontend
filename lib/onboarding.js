...
const pollAnonTopicsJob = async (jobId, anonUserId, { onProgress } = {}) => {
    ...
    while (true) {
        const statusUrl = `${API_PREFIX}/anon-topics/${encodeURIComponent(jobId)}?anonUserId=${encodeURIComponent(currentAnonUserId)}`;
        ...
        if (responseAnonUserId && responseAnonUserId !== currentAnonUserId) {
            currentAnonUserId = persistAnonUserId(responseAnonUserId) || responseAnonUserId;
        }
...
const pollAnonCourseJob = async (jobId, anonUserId, { onProgress } = {}) => {
    ...
    let currentAnonUserId = anonUserId;
    while (true) {
        const statusUrl = `${API_PREFIX}/anon-course-status/${encodeURIComponent(jobId)}?anonUserId=${encodeURIComponent(currentAnonUserId)}`;
        const res = await fetch(statusUrl, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        });
...
export async function generateAnonTopics(anonUserId, courseName, university, options = {}) {
    try {
        ...
        if (res.status === 202 || jobId) {
            ...
            return await pollAnonTopicsJob(jobId, effectiveAnonUserId, { onProgress });
        }
...
export async function generateAnonCourse(params = {}) {
    try {
        ...
        if (res.status === 202 || jobId) {
            ...
            const result = await pollAnonCourseJob(jobId, effectiveAnonUserId, { onProgress });
