...
async function proxyRequest(request, method) {
  try {
    ...
    const pathSegments = url.pathname.replace(/^\/api\/notifications\//, '');
    const backendUrl = new URL(`/notifications/${pathSegments}`, BASE_URL);
    ...
    console.error("Notifications API proxy error:", err);
