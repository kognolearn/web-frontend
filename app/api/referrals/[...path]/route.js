...
async function proxyRequest(request, method) {
  try {
    ...
    const pathSegments = url.pathname.replace(/^\/api\/referrals\//, '');
    const backendUrl = new URL(`/referrals/${pathSegments}`, BASE_URL);
    ...
    console.error("Referrals API proxy error:", err);
