...
async function proxyRequest(request, method) {
  try {
    ...
    const pathSegments = url.pathname.replace(/^\/api\/messaging\//, '');
    const backendUrl = new URL(`/messaging/${pathSegments}`, BASE_URL);
    ...
    console.error("Messaging API proxy error:", err);
