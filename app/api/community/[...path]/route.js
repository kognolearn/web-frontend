...
async function proxyRequest(request, method) {
  try {
    ...
    const pathSegments = url.pathname.replace(/^\/api\/community\//, '');
    const backendUrl = new URL(`/community/${pathSegments}`, BASE_URL);
    ...
    console.error("Community API proxy error:", err);
