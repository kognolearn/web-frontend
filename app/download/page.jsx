...
export default function DownloadPage() {
  ...
  const primaryOS = osInfo[detectedOS];
  const otherPlatforms = Object.entries(osInfo).filter(([key]) => key !== detectedOS);
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="rounded-2xl border border-white/10 dark:border-white/5 bg-[var(--surface-1)]/80 backdrop-blur-xl p-8 shadow-2xl">
          ...
          <a
            ...
            className="flex items-center justify-center gap-3 w-full px-6 py-4 rounded-xl bg-[var(--primary)] text-white font-medium hover:bg-[var(--primary)]/90 transition-all shadow-lg shadow-[var(--primary)]/20 mb-4"
          >
            <span className="text-white/90">{primaryOS.icon}</span>
            <span>Download for {primaryOS.name}</span>
          </a>
          ...
          {showAllPlatforms && (
            <div className="space-y-2 mb-6">
              {otherPlatforms.map(([key, info]) => (
                ...
                >
                  <span className="text-[var(--muted-foreground)]">{info.icon}</span>
                  <span className="text-sm">{info.name}</span>
                  <span className="ml-auto text-xs text-[var(--muted-foreground)]">{info.file}</span>
                </a>
              ))}
            </div>
          )}
